...
Data automation uses four workflows: Pages/provider updates (monthly on the 12th, 18th, 24th, and 28th), ACS (December–February), HCD (July and October), and market-brief draft review after successful Pages runs. No separate workflow is needed for inflation components or annual permit totals. A fifth workflow audits analytics tracking.

Ordinary code pushes now check the same health policy before deciding whether to refresh. Only providers with overdue/missing observations or repeated update failures are retried; healthy providers are skipped and keep their existing attempt state. Scheduled and manual refreshes still check every routine provider, while explicit `deploy_only` runs perform no provider downloads. ACS and HCD remain on their independent annual schedules.
