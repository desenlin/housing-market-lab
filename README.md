...
After a successful site and data validation, the separate **Prepare market brief for review** process evaluates four recurring questions: inflation-adjusted home values, residential permitting, for-sale inventory, and asking-rent change. The live **Current evidence snapshot** is automatic and deterministic; it is not an approved publication. An edition enters the archive only through maintainer review and merge of a draft pull request.

Each question designates one primary fact that controls its materiality decision. Supporting facts preserve calculations, component geography, and provenance but cannot independently trigger publication. The process recommends a new edition only when at least two questions have newer observation periods than the latest approved archive, or when one newer question's primary fact contains a material change, and the fact packet is not already represented there. Its headline and summary describe only the questions that qualified the edition. The workflow evaluates the exact commit validated by the successful Pages run rather than whichever commit happens to be current when the review starts. It can also be started manually with an optional issue month. Its force option bypasses only the advancement rule—not duplicate-edition, reused-packet, or future-period safeguards.

Materiality thresholds are fixed editorial reporting filters, not statistical-significance tests: 1% for the absolute year-over-year change in inflation-adjusted home values, 1.5% for typical asking rent, 5% for for-sale inventory, and 10% for matched Los Angeles metro year-to-date permit counts. Exact unrounded values determine eligibility. Directional answers may still report a smaller movement while identifying that it remains below the relevant threshold.
