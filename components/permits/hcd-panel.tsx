...
export function HcdPanel({ mapData, lensControl }: { mapData: MapData; lensControl: React.ReactNode }) {
  ...
  function toggleCity(id: string) { setSelectedIds(current => current.includes(id) ? current.filter(item => item !== id) : current.length < 3 ? [...current, id] : current); }
  function focusCity(id: string) { setSelectedIds(current => [id, ...current.filter(item => item !== id)].slice(0,3)); }
  ...
  const ranked = regions.filter(r => r.jurisdiction_type === "incorporated_city" && metricValue(r,index,metric) != null).sort((a,b) => metricValue(b,index,metric)! - metricValue(a,index,metric)! || a.name.localeCompare(b.name));
  const rank = ranked.findIndex(r => r.id === primary?.id) + 1;
  const value = primary ? metricValue(primary,index,metric) : null;
  const prior = primary && index > 0 ? metricValue(primary,index-1,metric) : null;
  const change = value == null || prior == null ? "Unavailable" : metric === "adu_share" ? `${number((value-prior)*100,1)} pp` : metric === "units_per_1000_stock" ? number(value-prior,1) : prior === 0 ? "Unavailable" : `${number((value/prior-1)*100,1)}%`;
  ...
  return <div className="permit-stack">
    ...
    <section className="kpi-grid permit-kpis" aria-label="Housing delivery summary">
      {[[primary?.name ?? "Select a jurisdiction",formatMetric(value,metric),`${info.short_label} · ${year}`], ["Change from one year earlier",change,metric === "adu_share" ? "Percentage-point change" : metric === "units_per_1000_stock" ? "Change in units per 1,000" : "Percent change; unavailable from a zero base"], ["City rank",rank > 0 ? `#${rank}` : "—",`Of ${ranked.length} reporting cities`], ["Observation status",value == null ? "Unavailable" : "Reported","Annual APR observations may be revised"]].map(([label,value,note]) => <Card className="kpi-card" key={label}><CardContent className="p-4"><p className="kpi-label">{label}</p><p className="kpi-value">{value}</p><p className="kpi-note">{note}</p></CardContent></Card>)}
    </section>
    ...
    <section className="analysis-grid hcd-analysis-grid">
      <Card className="chart-card hcd-trend"><CardHeader className="chart-header hcd-chart-header"><CardTitle>{info.label}<DefinitionHelp label={info.label} definition={info.definition} /></CardTitle><span className="metric-freshness">{data.years[0]}–{data.years.at(-1)}</span></CardHeader><CardContent>
      ...
      <Card className="ranking-card permit-ranking hcd-ranking"><CardHeader><p className="section-kicker">Cross-section</p><CardTitle>City ranking</CardTitle><p className="permit-chart-window">{info.short_label} · {year}</p><p className="permit-chart-window">Click to add or remove · up to three</p></CardHeader><CardContent className="ranking-list">{ranked.map((r,n) => <button type="button" aria-pressed={selectedIds.includes(r.id)} disabled={!selectedIds.includes(r.id) && chosen.length >= 3} className={`permit-rank-row ${selectedIds.includes(r.id) ? "active" : ""}`} key={r.id} onClick={() => toggleCity(r.id)}><span className="rank-number">{n+1}</span><span className="rank-name">{r.name}<small>{r.county}</small></span><strong>{formatMetric(metricValue(r,index,metric),metric)}</strong></button>)}</CardContent></Card>
    </section>
