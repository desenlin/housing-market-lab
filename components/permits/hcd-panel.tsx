...
export function HcdPanel({ mapData, lensControl }: { mapData: MapData; lensControl: React.ReactNode }) {
  ...
  return <div className="permit-stack">
    ...
    <section className="analysis-grid hcd-analysis-grid">
      {figureView === "trend" ? <Card className="chart-card hcd-trend"><CardHeader className="chart-header hcd-chart-header"><div><p className="section-kicker">Trend comparison</p><div className="metric-title-row"><CardTitle>{info.label}<DefinitionHelp label={info.label} definition={info.definition} /></CardTitle><DataThrough period={String(data.years.at(-1))} /></div><p className="hcd-chart-period">{data.years[0]}–{data.years.at(-1)}</p></div><div className="chart-options">{figureControl}</div></CardHeader><CardContent>
        {cityLegend}
      ...
      <Card className="chart-card hcd-composition"><CardHeader className="chart-header hcd-chart-header"><div><p className="section-kicker">Housing composition</p><div className="metric-title-row"><CardTitle>Housing type<DefinitionHelp label="Housing types" definition="Completed units by reported structure type. ADUs are counted once in a separate category; unknown types remain Other / unspecified."/></CardTitle><DataThrough period={String(data.years.at(-1))} /></div><p className="hcd-chart-period">Completed units · {year}</p></div><div className="chart-options">{figureControl}</div></CardHeader><CardContent>
        {cityLegend}
      ...
      <Card className="ranking-card hcd-ranking"><CardHeader>
        <div className="ranking-title"><div><p className="section-kicker">Place</p><CardTitle>City ranking</CardTitle></div><RankingSort label="HCD ranking sort" value={rankBy} onChange={setRankBy} /></div>
