...
City rankings support **Level**, **Change**, and, for monthly data, **Last 12 months**. Count changes are absolute differences in authorized units from the same calendar month or annual total one year earlier, including when the prior value is zero. Share changes use percentage points; normalized-rate changes use units per 1,000. The rolling option sums the 12 months ending at the selected map/ranking date, requires all 12 observations, and compares that total with the preceding 12-month total in its change column (requiring 24 observations). Rolling shares divide summed 5+-unit authorizations by summed total authorizations; rolling normalized rates divide summed total authorizations by the fixed ACS stock denominator. Missing observations are never treated as zero. These are gross authorizations, not net additions to housing stock; HCD completions also do not deduct demolitions. Annual BPS and HCD levels already cover a full reporting year, so they do not have a duplicate rolling-year option.
...
Four GitHub Actions workflows cover all current data families. The monthly Pages workflow refreshes Zillow, Redfin, Realtor.com, all twenty BLS CPI series, and Census BPS on the 12th, 18th, 24th, and 28th. ACS and HCD keep independent annual schedules. The fourth data workflow prepares market-brief drafts for review after a successful Pages run; analytics tracking has a separate audit workflow. Ordinary code pushes first assess coverage and retry only routine providers with missing/overdue observations or repeated update failures before enforcing health. Providers skipped by this plan retain their attempt state. Explicit deployment-only runs skip these downloads, and ACS/HCD stay on their annual schedules.
