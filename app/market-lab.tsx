...
import { DATA_CHECKS_DEFINITION } from "@/lib/data-status";
...
export default function MarketLab() {
  ...
  const { manifest: hcdManifest, error: hcdManifestError } = useHcdManifest();
  ...
  const [supplyView, setSupplyView] = useState("activity");
  ...
  const mappedCityCount = Object.values(maps.city.counties)
    .reduce((sum, item) => sum + item.regions.length, 0);
  const mappedZipCount = Object.values(maps.zip.counties)
    .reduce((sum, item) => sum + item.regions.length, 0);
      ...
      <Tabs value={mainTab} onValueChange={setMainTab} className="page-shell">
        ...
        <TabsContent value="methods" className="space-y-5">
          ...
          <section className="method-grid">
            ...
            <MethodCard title="Building permits"><p>The U.S. Census Bureau Building Permits Survey reports new privately owned housing units authorized by permit-issuing jurisdictions. The lab groups units into single-unit, 2–4-unit, and 5+-unit structures and shows annual history from 1980 and comparable local monthly history from 2022.</p><p>Monthly estimates are official Census observations. Current-year figures are preliminary and may change as late reports and corrections arrive. Historical monthly estimates preserve the local monthly observations available when annual totals were released. Census does not benchmark local monthly figures to final annual totals, so the two may differ. Only annual totals are labeled final after the yearly revision cycle. <a href="https://www.census.gov/construction/bps/methodology.html" target="_blank" rel="noreferrer">Census revision methodology</a>.</p><p>Census-imputed flags identify published observations that include estimated activity for missing reports; both monthly and annual data can include imputation. These source characteristics are separate from the Lab’s processing checks and do not assign a reliability ranking across providers. Permit authorization is an early production indicator, not a housing start or completion.</p></MethodCard>
            ...
            <MethodCard title="Update monitoring"><p>Monthly market and inflation sources are checked several times each month. Observation dates are monitored against source-specific release windows, including the later Census local-permit schedule. Annual ACS, permit totals, and HCD data use annual freshness windows.</p><p>Persistent update failures or overdue observations are flagged for maintenance while releases that pass validation remain available. A successful check can leave a release unchanged. These operational checks do not rate statistical accuracy or remove a source’s revision caveats.</p><p>Permit history is checked for corrections quarterly, and ACS estimates are rechecked in February. Map vintages, metro population ranks, and the fixed ACS housing-stock denominator receive an annual reference review; they do not advance automatically with a new housing-context release.</p></MethodCard>
            <MethodCard title="Technical design"><p>The site is a static export with no database, application server, paid API, or paid map service. Provider updates pass validation before release, and GitHub Pages serves the resulting files.</p><p>ACS checks its annual release window in December, January, and February; February also checks corrections to the current vintage. ACS retrievals request only selected variables, publish only local estimates and margins of error, and reuse the shared map geometry. HCD checks for source changes in July and October and supports manual refreshes. It retrieves county-filtered aggregates, validates annual stages, and retains a rollback release. Published JSON files remain below 1 MB, and a total storage limit prevents unbounded growth.</p></MethodCard>
          </section>
          <Card className="disclaimer-card">
            ...
            <CardContent className="method-copy">
              ...
              <p>Third-party data remain subject to their providers’ licenses and terms. This project does not grant commercial-use rights to Zillow, Redfin, Realtor.com, BLS, Census, HCD, or OpenStreetMap data.</p>
            </CardContent>
          </Card>
          <Card id="current-release-provenance" className="provenance-card">
            ...
            <CardContent>
              <p className="provenance-intro">Coverage identifies the newest observation in each source. <strong>Data checks completed</strong> <DefinitionHelp label="Data checks completed" definition={DATA_CHECKS_DEFINITION} /> identifies when the Lab accepted the snapshot after checking source files, data structure, dates, coverage, and calculation inputs. Every provider, including monthly permits, passes these checks. This is a processing date, not a certification of statistical accuracy or protection against source revisions.</p>
              <dl className="provenance-grid">
                <div><dt>Zillow prices &amp; rents</dt><dd>Through {shortDate(latestObservation(Object.values(manifest.latest_observations)) ?? manifest.release)}<small>Data checks completed {validationDate(manifest.created_at)}</small></dd></div>
                <div><dt>Redfin market activity</dt><dd>{redfinManifest ? `Through ${shortDate(latestObservation(Object.values(redfinManifest.latest_observations)) ?? redfinManifest.release)}` : "Unavailable"}<small>{redfinManifest ? `Data checks completed ${validationDate(redfinManifest.created_at)}` : ""}</small></dd></div>
                <div><dt>Realtor inventory</dt><dd>{realtorManifests.inventory ? `Through ${shortDate(latestObservation(Object.values(realtorManifests.inventory.latest_observations)) ?? realtorManifests.inventory.release)}` : "Unavailable"}<small>{realtorManifests.inventory ? `Data checks completed ${validationDate(realtorManifests.inventory.created_at)}` : ""}</small></dd></div>
                <div><dt>Realtor Hotness</dt><dd>{realtorManifests.hotness ? `Through ${shortDate(latestObservation(Object.values(realtorManifests.hotness.latest_observations)) ?? realtorManifests.hotness.release)}` : "Unavailable"}<small>{realtorManifests.hotness ? `Data checks completed ${validationDate(realtorManifests.hotness.created_at)}` : ""}</small></dd></div>
                <div><dt>BLS CPI</dt><dd>{cpiManifest ? `Through ${shortDate(latestObservation(Object.values(cpiManifest.series).map((item) => item.latest_observation)) ?? cpiManifest.release)}` : "Unavailable"}<small>{cpiManifest ? `Data checks completed ${validationDate(cpiManifest.created_at)}` : ""}</small></dd></div>
                <div><dt>ACS housing context</dt><dd>{acsManifest ? `${acsManifest.periods[1]} five-year estimates` : "Unavailable"}<small>{acsManifest ? `Data checks completed ${validationDate(acsManifest.created_at)}` : ""}</small></dd></div>
                <div><dt>Final annual permit totals</dt><dd>{permitManifests ? `Through ${permitManifests.history.latest_final_year}` : "Loading…"}<small>{permitManifests ? `Data checks completed ${validationDate(permitManifests.history.created_at)}` : ""}</small></dd></div>
                <div><dt>Historical monthly permit estimates</dt><dd>{permitManifests ? `Through ${shortDate(permitManifests.history.latest_final_month ?? `${permitManifests.history.latest_final_year}-12`)}` : "Loading…"}<small>{permitManifests ? `Data checks completed ${validationDate(permitManifests.history.created_at)}` : ""}</small></dd></div>
                <div><dt>Current monthly permit estimates</dt><dd>{permitManifests?.provisional.latest_observation ? `Through ${shortDate(permitManifests.provisional.latest_observation)}` : "Loading…"}<small>{permitManifests ? `Data checks completed ${validationDate(permitManifests.provisional.created_at)}` : ""}</small></dd></div>
                <div><dt>HCD housing delivery</dt><dd>{hcdManifest ? `Annual through ${hcdManifest.latest_year}` : hcdManifestError ? "Unavailable" : "Loading…"}<small>{hcdManifest ? `Data checks completed ${validationDate(hcdManifest.created_at)}` : ""}</small></dd></div>
                <div><dt>Mapped geography</dt><dd>{mappedCityCount} city/community · {mappedZipCount} ZCTA boundaries</dd></div>
                <div><dt>Zillow observation coverage</dt><dd>{manifest.counts.city} city/community · {manifest.counts.zip} ZIP · {manifest.counts.metro} metro series</dd></div>
              ...
              <details className="provenance-technical">
                ...
                <dl className="fingerprint-list">
                  ...
                  {hcdManifest && <div><dt>HCD housing delivery</dt><dd><span>Release {hcdManifest.release}</span><code>{hcdManifest.bundle_sha256}</code></dd></div>}
                  {permitManifests && <div><dt>Permit history</dt><dd><span>Release {permitManifests.history.release}</span><code>{permitManifests.history.bundle_sha256}</code></dd></div>}
                  {permitManifests && <div><dt>Current monthly permit estimates</dt><dd><span>Release {permitManifests.provisional.release}</span><code>{permitManifests.provisional.bundle_sha256}</code></dd></div>}
                </dl>
              </details>
              <p className="attribution">{manifest.attribution}. {redfinManifest?.attribution} {realtorManifests.inventory?.attribution ?? realtorManifests.hotness?.attribution} {cpiManifest?.attribution} {acsManifest?.attribution} {permitManifests?.history.attribution} Housing delivery: California HCD Annual Progress Reports. Map data © OpenStreetMap contributors. This independent academic visualization is not endorsed by Zillow Group, Redfin, Realtor.com, BLS, Census, HCD, HUD, or OpenStreetMap.</p>
              <div className="source-links">
                ...
                <a className="source-link" href={hcdManifest?.data_page ?? "https://data.ca.gov/dataset/housing-element-annual-progress-report-apr-data-by-jurisdiction-and-year"} target="_blank" rel="noreferrer">View California HCD APR source data <ExternalLink /></a>
